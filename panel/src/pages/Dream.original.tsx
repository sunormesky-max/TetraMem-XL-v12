...
export default function Dream() {
  ...
  const handleRunDream = useCallback(() => {
    setIsRunning(true)
    setTimeout(() => {
      const newDream: DreamRecord = {
        id: history.length + 1,
        time: '刚刚',
        memories: Math.floor(Math.random() * 1500 + 500),
        phases: ['light', 'medium', 'deep'].slice(
          0,
          Math.floor(Math.random() * 3) + 1
        ) as DreamPhase[],
        status: 'completed',
        weakened: Math.floor(Math.random() * 30),
        consolidated: Math.floor(Math.random() * 1500 + 400),
      }
