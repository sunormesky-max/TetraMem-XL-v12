...
mod tests {
    ...
    fn find_analogies_by_energy() {
        ...
        assert!(
            !results.is_empty(),
            "m1 and m3 have same data, should be analogous"
        );
    ...
    fn find_associations_via_hebbian() {
        ...
        assert!(
            !results.is_empty(),
            "should find associated nodes via hebbian"
        );
    }
    ...
    fn infer_chain_connects_memories() {
        ...
        let results = ReasoningEngine::infer_chain(&u, &h, mems[0].anchor(), mems[2].anchor(), 10);
    ...
    fn discover_via_pulse() {
        ...
        assert!(
            !results.is_empty(),
            "pulse exploration should discover connections"
        );
    }
