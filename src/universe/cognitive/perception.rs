...
impl std::fmt::Display for PerceptionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PerceptionError::InsufficientBudget {
                requested,
                available,
            } => {
                write!(
                    f,
                    "insufficient perception budget: requested={}, available={}",
                    requested, available
                )
            }
            PerceptionError::OverAllocate { requested, max } => {
                write!(
                    f,
                    "single perception exceeds max: requested={}, max={}",
                    requested, max
                )
            }
            PerceptionError::InvalidReturn { returned, spent } => {
                write!(
                    f,
                    "return exceeds spent: returned={}, spent={}",
                    returned, spent
                )
            }
...
impl PerceptionBudget {
    pub fn new(total_universe_energy: f64) -> Self {
        let total_budget =
            (total_universe_energy * DEFAULT_BUDGET_RATIO).max(MIN_PERCEPTION_ENERGY);
        Self {
            total_budget,
            allocated: 0.0,
            spent: 0.0,
    ...
    pub fn settle(
        &mut self,
        alloc: PerceptionAlloc,
        actual_cost: f64,
    ) -> Result<f64, PerceptionError> {
...
mod tests {
    ...
    fn insufficient_budget_rejected() {
        ...
        assert!(matches!(
            result,
            Err(PerceptionError::InsufficientBudget { .. })
        ));
    }
