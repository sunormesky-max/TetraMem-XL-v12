...
impl std::fmt::Display for CrystalReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.new_crystals,
            self.new_super_crystals,
            self.total_crystals,
            self.total_super_crystals,
            self.energy_locked
        )
    }
}
...
impl CrystalEngine {
    ...
    pub fn detect_phase_transition(
        ...
        let avg_weight = if strong.is_empty() {
            0.0
        } else {
            total_weight / strong.len() as f64
        };
    ...
    pub fn decay_unused(&mut self, active_nodes: &HashSet<Coord7D>) -> usize {
        ...
        self.channels
            .retain(|(a, b), _| active_nodes.contains(a) && active_nodes.contains(b));
        before - self.channels.len()
    }

    pub fn crystal_path(&self, start: &Coord7D, end: &Coord7D, max_hops: usize) -> Vec<Coord7D> {
...
mod tests {
    ...
    fn crystallize_creates_channels() {
        ...
        assert!(
            ab_weight > 0.0,
            "hebbian should have edges, got weight {}",
            ab_weight
        );
    ...
    fn super_crystal_for_very_strong() {
        ...
        assert!(
            report.total_super_crystals > 0,
            "50x reinforce should create super crystal"
        );
    }
