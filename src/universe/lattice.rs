...
impl Projection {
    ...
    pub fn verify_bcc(universe: &DarkUniverse) -> BccVerification {
        let active: Vec<Coord7D> = universe
            ...
            .keys()
            .filter(|c| {
                universe.get_node(c).map_or(false, |n| !n.energy().is_empty())
            })
            .copied()
            .collect();
        ...
        let odd_half_integer = active
            .iter()
            .filter(|c| c.is_odd())
            .all(|c| {
                let p = Self::to_3d(c);
                p.iter().all(|v| (2.0 * v - (2.0 * v).round()).abs() < 1e-10)
            });
...
impl fmt::Display for BccVerification {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "  Odd at half-int:   {}",
            if self.odd_at_half_integer { "PASS" } else { "FAIL" }
        )?;
        writeln!(
            f,
...
mod tests {
    ...
    fn bcc_neighbors_present_filtered() {
        ...
        for i in 0..5 {
            u.materialize_uniform(bcc_coords[i], 10.0).unwrap();
        }
    ...
    fn bcc_tetrahedra_around_anchor() {
        ...
        assert!(tets.len() >= 1, "should find at least 1 BCC tetrahedron");
        for tet in &tets {
            assert!(tet.has_volume());
            assert!(tet.exists_in(&u));
        }
    ...
    fn bcc_emergence_projection() {
        ...
        assert!(tets.len() >= 1, "BCC tetrahedra should emerge");
        for tet in &tets {
            assert!(tet.has_volume());
            assert!(tet.exists_in(&u));
        }
    ...
    fn bcc_all_with_grid() {
        ...
        assert!(bcc_tets.len() > 0, "should find BCC tetrahedra");
        assert!(face_tets.len() > 0, "should find face tetrahedra");
    ...
    fn bcc_verification_full_grid() {
        ...
        assert!(v.even_at_integer, "Even nodes should be at integer positions");
        assert!(v.odd_at_half_integer, "Odd nodes should be at half-integer positions");
        assert!(
    ...
    fn bcc_tetrahedra_all_mixed_parity() {
        ...
        let face_tets = Tetrahedron::find_face_all(&u);
        for tet in &face_tets {
            assert!(!tet.is_mixed_parity(), "face tetrahedra must be single parity");
        }
    }
    ...
    fn projection_bcc_vs_7d_distance() {
        ...
        assert!((d_7d - d_3d - 1.0).abs() < 1e-10, "dark dims contribute 1.0");
    }
}
