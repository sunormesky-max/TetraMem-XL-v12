...
impl ClusterManager {
    ...
    pub async fn remove_peer(&mut self, node_id: u64) -> Result<(), String> {
        ...
        let voter_ids: std::collections::BTreeSet<u64> = remaining.keys().copied().collect();
    ...
    pub async fn quorum_propose(
        ...
        match self.active_quorum {
            ...
            Some(ref q) => Err(format!(
                "quorum not reached: {}/{} confirmations",
                q.confirming_count(),
                q.quorum_threshold
            )),
            None => Err("no active quorum, call start_energy_quorum first".to_string()),
        }
    }
}
