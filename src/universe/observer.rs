...
impl SelfRegulator {
    ...
    pub fn execute_expansion(
            ...
            for dy in -r..=r {
                for dz in -r..=r {
                    ...
                    if !universe.contains(&c) {
                        if universe.materialize_biased(c, 50.0, 0.5).is_ok() {
                            new_nodes += 1;
                        }
                    }
                    let ox = center[0] + dx;
                    ...
                    if !universe.contains(&o) {
                        if universe.materialize_biased(o, 40.0, 0.3).is_ok() {
                            new_nodes += 1;
                        }
                    }
                }
        ...
        RegulatorAction {
            action_type: RegulatorActionType::ExpandLattice,
            description: format!(
                "expanded: +{:.0} energy, +{} nodes",
                additional, new_nodes
            ),
        }
    }
    ...
    fn find_center(universe: &DarkUniverse) -> [i32; 3] {
        ...
        let sum: [i64; 3] = coords.iter().map(|c| c.physical()).fold([0i64; 3], |acc, p| {
...
mod tests {
    ...
    fn setup_system() -> (DarkUniverse, HebbianMemory, Vec<MemoryAtom>) {
        ...
        let mem = MemoryCodec::encode(&mut u,
    ...
    fn regulator_no_action_when_healthy() {
        ...
        assert!(actions.iter().any(|a| a.action_type == RegulatorActionType::None));
    }
    ...
    fn regulator_prunes_hebbian() {
        ...
        let mut params = RegulatorParams::default();
        params.hebbian_edge_limit = 5;
        let regulator = SelfRegulator::with_params(params);
        let report = UniverseObserver::inspect(&u, &h, &mems);
        let actions = regulator.regulate(&report, &mut h);
        assert!(actions.iter().any(|a| a.action_type == RegulatorActionType::PruneHebbian));
    ...
    fn regulator_suggests_expansion() {
        ...
        u.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 90.0, 0.6).unwrap();
        let h = HebbianMemory::new();
    ...
    fn frontier_computed_correctly() {
        let mut u = DarkUniverse::new(10000.0);
        u.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 100.0, 0.6).unwrap();
        let report = UniverseObserver::inspect(&u, &HebbianMemory::new(), &[]);
        assert!(report.frontier_size > 0, "single node should have frontier neighbors");
    }
}
