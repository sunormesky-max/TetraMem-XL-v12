...
impl AutoScaler {
                ...
                for dz in -r..=r {
                    ...
                    if !universe.contains(&ec)
                        && universe
                            .materialize_biased(
                                ec,
                                self.config.node_energy_amount,
                                self.config.node_physical_ratio,
                            )
                            .is_ok()
                    {
                        nodes_added += 1;
    ...
    pub fn scale_down(&self, universe: &mut DarkUniverse, memories: &[MemoryAtom]) -> ScaleReport {
    ...
    pub fn frontier_expansion(&self, universe: &mut DarkUniverse, max_new: usize) -> ScaleReport {
        let coords = universe.coords();
...
mod tests {
    ...
    fn evaluate_high_utilization() {
        ...
        u.materialize_biased(Coord7D::new_even([0; 7]), 85.0, 0.6)
            .unwrap();
    ...
    fn auto_scale_triggers_on_high_util() {
        ...
        u.materialize_biased(Coord7D::new_even([0; 7]), 180.0, 0.6)
            .unwrap();
        let h = HebbianMemory::new();
    ...
    fn scale_preserves_memories() {
        ...
        let h = HebbianMemory::new();
        let scaler = AutoScaler::new();
        scaler.auto_scale(&mut u, &h, std::slice::from_ref(&mem));
