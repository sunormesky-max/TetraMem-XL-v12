...
impl std::fmt::Display for TopologyReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Topo[{} comps:{} cycles:{} tetra:{} bridges:{} isolated:{} coord:{:.2}]",
            self.betti, self.connected_components, self.cycles_detected,
            self.tetrahedra_count, self.bridging_nodes, self.isolated_nodes,
...
impl TopologyEngine {
    ...
    fn find_connected_components(nodes: &[Coord7D], node_set: &HashSet<Coord7D>) -> Vec<Vec<Coord7D>> {
    ...
    fn compute_coordination(nodes: &[Coord7D], node_set: &HashSet<Coord7D>) -> HashMap<Coord7D, usize> {
    ...
    fn count_tetrahedra(nodes: &[Coord7D], node_set: &HashSet<Coord7D>) -> usize {
        ...
        for node in nodes {
            if node.is_even() {
                for n in Lattice::bcc_neighbor_coords(node) {
                    if node_set.contains(&n) && !seen.contains(&(*node, n)) && !seen.contains(&(n, *node)) {
                        seen.insert((*node, n));
                        count += 1;
                    }
                }
...
mod tests {
    ...
    fn analyze_connected_lattice() {
        ...
        assert_eq!(report.connected_components, 1, "continuous lattice should be one component");
    ...
    fn h0_counts_components() {
        ...
        u.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();
        u.materialize_biased(Coord7D::new_even([100, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();

        let report = TopologyEngine::analyze(&u);
        assert_eq!(report.connected_components, 2, "two distant nodes = 2 components");
    }
    ...
    fn shortest_path_no_route() {
        ...
        let path = TopologyEngine::find_shortest_path(&u,
    ...
    fn bridging_nodes_on_partial_lattice() {
        let mut u = DarkUniverse::new(100_000.0);
        u.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();
        u.materialize_biased(Coord7D::new_even([1, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();
        u.materialize_biased(Coord7D::new_even([2, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();
