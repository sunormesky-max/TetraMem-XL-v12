...
impl DarkUniverse {
    ...
    pub fn transfer_energy(
        ...
        let should_remove = self
            .nodes
            .get(from)
            .map_or(false, |n| n.energy.is_empty());
...
mod tests {
    ...
    fn even_and_odd_coexist() {
        let mut u = DarkUniverse::new(1000.0);
        u.materialize_uniform(Coord7D::new_even([0; 7]), 50.0).unwrap();
        u.materialize_uniform(Coord7D::new_odd([0; 7]), 50.0).unwrap();
    ...
    fn stats_physical_dark_split() {
        ...
        u.materialize_biased(Coord7D::new_even([0; 7]), 100.0, 0.8).unwrap();
        u.materialize_biased(Coord7D::new_even([1; 7]), 100.0, 0.2).unwrap();
