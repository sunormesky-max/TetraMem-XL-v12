    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MemoryError::InvalidDataRange { index, value } => {
                write!(
                    f,
                    "data[{}] = {:.1} would make dimension negative (min base = {:.1})",
                    index, value, DATA_OFFSET
                )
            }
        }
    }
...
impl MemoryAtom {
    ...
    pub fn from_parts_with_importance(
        vertices: [Coord7D; 4],
        data_dim: usize,
        physical_base: f64,
        created_at: u64,
...
impl MemoryCodec {
    ...
    pub fn encode(
        ...
        for (idx, &v) in data.iter().enumerate() {
            ...
            if base + v < MIN_DIM_VALUE {
                return Err(MemoryError::InvalidDataRange {
                    index: idx,
                    value: v,
                });
            }
        ...
        for (node_idx, _coord) in positions.iter().enumerate() {
            ...
            for d in dims.iter_mut().take(PHYSICAL_DIM) {
                *d = physical_base + DATA_OFFSET;
            }
            for (d, dim_val) in dims.iter_mut().enumerate() {
                let data_idx = node_idx * DIM + d;
                if data_idx < data.len() {
                    *dim_val += data[data_idx];
                }
            }
    ...
    pub fn decode(universe: &DarkUniverse, atom: &MemoryAtom) -> Result<Vec<f64>, MemoryError> {
        let mut result = Vec::with_capacity(atom.data_dim);

        for (node_idx, coord) in atom.vertices.iter().enumerate() {
            let node = universe.get_node(coord).ok_or(MemoryError::NodeNotFound)?;
            let dims = node.energy().dims();

            for (d, &dim_val) in dims.iter().enumerate() {
                let data_idx = node_idx * DIM + d;
    ...
    pub fn encode_with_importance(
        universe: &mut DarkUniverse,
        anchor: &Coord7D,
        data: &[f64],
        importance: f64,
    ) -> Result<MemoryAtom, MemoryError> {
        let mut atom = Self::encode(universe, anchor, data)?;
        atom.importance = importance.clamp(0.0, 1.0);
        Ok(atom)
    }
...
mod tests {
    ...
    fn erase_frees_energy() {
        ...
        assert!(
            after_erase > after_encode,
            "energy should increase after erase"
        );
