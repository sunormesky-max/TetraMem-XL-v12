...
use prometheus::{self, Encoder, IntCounter, IntGauge, Histogram, HistogramOpts, Opts, TextEncoder};

lazy_static! {
    pub static ref NODES_TOTAL: IntGauge = IntGauge::new(
        "tetramem_nodes_total",
        "Total number of nodes in universe"
    ).unwrap();

    pub static ref NODES_MANIFESTED: IntGauge = IntGauge::new(
        "tetramem_nodes_manifested",
        "Number of manifested nodes"
    ).unwrap();

    pub static ref NODES_DARK: IntGauge = IntGauge::new(
        "tetramem_nodes_dark",
        "Number of dark nodes"
    ).unwrap();

    pub static ref ENERGY_TOTAL: IntGauge = IntGauge::with_opts(
        Opts::new("energy_total", "Total energy in universe")
            .namespace("tetramem")
    ).unwrap();

    pub static ref ENERGY_ALLOCATED: IntGauge = IntGauge::with_opts(
        Opts::new("energy_allocated", "Allocated energy")
            .namespace("tetramem")
    ).unwrap();

    pub static ref ENERGY_AVAILABLE: IntGauge = IntGauge::with_opts(
        Opts::new("energy_available", "Available energy")
            .namespace("tetramem")
    ).unwrap();

    pub static ref MEMORIES_TOTAL: IntGauge = IntGauge::new(
        "tetramem_memories_total",
        "Total stored memories"
    ).unwrap();

    pub static ref HEBBIAN_EDGES: IntGauge = IntGauge::new(
        "tetramem_hebbian_edges",
        "Total Hebbian edges"
    ).unwrap();

    pub static ref API_REQUESTS_TOTAL: IntCounter = IntCounter::new(
        "tetramem_api_requests_total",
        "Total API requests"
    ).unwrap();
    ...
    pub static ref API_PULSE_TOTAL: IntCounter = IntCounter::new(
        "tetramem_api_pulse_total",
        "Total pulse operations"
    ).unwrap();

    pub static ref API_DREAM_TOTAL: IntCounter = IntCounter::new(
        "tetramem_api_dream_total",
        "Total dream cycle operations"
    ).unwrap();
...
pub fn init_metrics() {
    ...
    registry.register(Box::new(NODES_MANIFESTED.clone())).unwrap();
    registry.register(Box::new(NODES_DARK.clone())).unwrap();
    registry.register(Box::new(ENERGY_TOTAL.clone())).unwrap();
    registry.register(Box::new(ENERGY_ALLOCATED.clone())).unwrap();
    registry.register(Box::new(ENERGY_AVAILABLE.clone())).unwrap();
    ...
    registry.register(Box::new(API_ENCODE_TOTAL.clone())).unwrap();
    registry.register(Box::new(API_DECODE_TOTAL.clone())).unwrap();
    registry.register(Box::new(API_PULSE_TOTAL.clone())).unwrap();
    registry.register(Box::new(API_DREAM_TOTAL.clone())).unwrap();
