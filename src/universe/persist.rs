...
impl std::fmt::Display for PersistReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.nodes_serialized, self.edges_serialized,
            self.memories_serialized, self.crystals_serialized, self.bytes_written
        )
    }
}
...
impl PersistEngine {
    pub fn serialize(
        ...
        let mem_snapshots: Vec<MemorySnapshot> = memories.iter().map(|m| {
        ...
        let channels: Vec<ChannelSnapshot> = crystal.all_channels().iter().map(|((a, b), ch)| {
            ChannelSnapshot {
                a: a.basis(),
                a_even: a.is_even(),
                b: b.basis(),
    ...
    pub fn deserialize(snapshot: &UniverseSnapshot) -> Result<(DarkUniverse, HebbianMemory, Vec<MemoryAtom>, CrystalEngine), PersistError> {
        ...
        for es in &snapshot.hebbian_edges {
            let a = if es.a_even { Coord7D::new_even(es.a) } else { Coord7D::new_odd(es.a) };
            let b = if es.b_even { Coord7D::new_even(es.b) } else { Coord7D::new_odd(es.b) };
        ...
        for cs in &snapshot.crystal_channels {
            let a = if cs.a_even { Coord7D::new_even(cs.a) } else { Coord7D::new_odd(cs.a) };
            let b = if cs.b_even { Coord7D::new_even(cs.b) } else { Coord7D::new_odd(cs.b) };
    ...
    pub fn from_json(json: &str) -> Result<(DarkUniverse, HebbianMemory, Vec<MemoryAtom>, CrystalEngine), PersistError> {
...
mod tests {
    ...
    fn roundtrip_preserves_conservation() {
        ...
        assert!(u2.verify_conservation(), "conservation must survive roundtrip");
    }
