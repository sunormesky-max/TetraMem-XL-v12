...
impl std::fmt::Display for RegulationReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.stress_level, self.entropy,
...
impl RegulationEngine {
    ...
    pub fn regulate(
        ...
        if pressure.imbalance > self.pressure_threshold {
            ...
            actions.push(RegAction {
                ...
                detail: format!("drained {:.1} from dim {} (pressure {:.2})", drained, dim, pressure.max_pressure_value),
            });
        }
        ...
        if entropy < self.entropy_target * 0.5 {
            ...
            actions.push(RegAction {
                ...
                detail: format!("entropy {:.3} below target {:.3}, decay applied", entropy, self.entropy_target),
            });
        }
        ...
        if stress > self.stress_threshold {
            ...
            if scale_report.nodes_added > 0 || scale_report.energy_expanded_by > 0.0 {
                actions.push(RegAction {
                    ...
                    detail: format!("+{} nodes +{:.0} energy", scale_report.nodes_added, scale_report.energy_expanded_by),
                });
            }
        }
    ...
    fn redistribute_from_dimension(&self, universe: &mut DarkUniverse, dim: usize, fraction: f64) -> f64 {
...
mod tests {
    ...
    fn setup() -> (DarkUniverse, HebbianMemory, CrystalEngine, Vec<MemoryAtom>) {
        ...
        let m = MemoryCodec::encode(&mut u,
    ...
    fn entropy_nonzero_with_nodes() {
        ...
        assert!(entropy > 0.0, "system with both manifested and dark nodes should have entropy");
    }
    ...
    fn stress_high_on_overutilization() {
        ...
        u.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 95.0, 0.9).unwrap();
        let h = HebbianMemory::new();
        let mems = Vec::new();
