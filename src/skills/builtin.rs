...
macro_rules! skill_impl {
    ($name:expr, $version:expr, $desc:expr, $input:expr, $output:expr, $exec:expr) => {
        ...
        impl crate::skills::types::Skill for Skill {
            ...
            fn execute(&self, ctx: &mut crate::skills::types::SkillContext, args: &Value) -> Result<Value, crate::skills::types::SkillError> {
                $exec(ctx, args)
            }
        }
    };
        ...
        |ctx: &mut SkillContext, args: &Value| {
            ...
            match MemoryCodec::encode(ctx.universe, &anchor, &data) {
                Ok(mem) => {
                    ...
                    Ok(json!({"anchor": s, "dimensions": dim, "conservation_ok": ctx.universe.verify_conservation()}))
                }
...
pub mod fire_pulse {
    ...
    skill_impl!(
        ...
        |ctx: &mut SkillContext, args: &Value| {
            ...
            Ok(json!({"visited": r.visited_nodes, "activation": r.total_activation, "paths_recorded": r.paths_recorded}))
        }
    );
}
...
pub mod run_dream {
    ...
    skill_impl!(
        ...
        |ctx: &mut SkillContext, _args: &Value| {
            ...
            Ok(json!({"edges_before": r.hebbian_edges_before, "edges_after": r.hebbian_edges_after, "weight_delta": r.weight_after - r.weight_before}))
        }
    );
}
...
pub mod analyze_topology {
    use super::*;
    skill_impl!(
        "analyze_topology", "1.0.0",
        "Compute Betti numbers and topology",
        json!({}),
        json!({"betti": "String", "components": "usize", "euler": "i64"}),
        |ctx: &mut SkillContext, _args: &Value| {
            let r = TopologyEngine::analyze(ctx.universe);
            Ok(json!({"betti": format!("{}", r.betti), "components": r.connected_components, "euler": r.betti.euler_characteristic()}))
        }
    );
}
...
pub mod trace_associations {
    ...
    skill_impl!(
        ...
        |ctx: &mut SkillContext, args: &Value| {
            ...
            let results = ReasoningEngine::find_associations(ctx.universe, ctx.hebbian, ctx.crystal, &anchor, max_hops);
            let out: Vec<Value> = results.iter().map(|a| json!({"targets": a.targets, "confidence": a.confidence, "hops": a.hops})).collect();
...
fn parse_coord(args: &Value, key: &str) -> Result<Coord7D, SkillError> {
    match args.get(key).and_then(|v| v.as_array()) {
        Some(a) if a.len() == 3 => {
            let c: Result<Vec<i32>, _> = a.iter().map(|v| v.as_i64().map(|n| n as i32).ok_or(())).collect();
            match c {
                Ok(v) => Ok(Coord7D::new_even([v[0], v[1], v[2], 0, 0, 0, 0])),
                Err(_) => Err(SkillError::new("parse", format!("{} must be 3 integers", key))),
            }
        }
        _ => Err(SkillError::new("parse", format!("{} must be array of 3 integers", key))),
    }
}
...
fn parse_pulse_type(args: &Value) -> Result<PulseType, SkillError> {
    match args.get("pulse_type").and_then(|v| v.as_str()) {
        ...
        _ => Err(SkillError::new("parse", "pulse_type must be reinforcing/exploratory/cascade")),
    }
}
