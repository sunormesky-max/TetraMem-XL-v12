...
fn main() {
    ...
    println!("  轮次: 1000");
    println!("  记忆: {}  节点: {}  利用率: {:.1}%", mems.len(), stats.active_nodes, stats.utilization * 100.0);
    println!("  总能量: {:.0} (初始10000 → 扩展+{:.0})", u.total_energy(), total_energy_expanded);
    println!("  扩展事件: {}  新增节点: {}  编码失败: {}", total_scale_events, total_nodes_added, encode_failures);
    println!("  守恒: {}  健康: {}", if u.verify_conservation() { "✓" } else { "✗" }, health.health_level().as_str());
    ...
    for (i, mem) in mems.iter().enumerate() {
        if let Ok(decoded) = MemoryCodec::decode(&u, mem) {
            let err = datasets[i].iter().zip(decoded.iter()).map(|(a, b)| (a - b).abs()).fold(0.0f64, f64::max);
            max_err = max_err.max(err);
            if err > 1e-10 { corrupt_count += 1; }
        } else {
            corrupt_count += 1;
        }
    }
    println!("  记忆精度: 最大误差 {:.2e}  损坏: {}/{}", max_err, corrupt_count, mems.len());
    ...
    for i in 0..10i32 {
        ...
        match MemoryCodec::encode(&mut u2, &anchor, &data) {
            Ok(mem) => { mems2.push(mem); datasets2.push(data); },
            Err(_) => {
                let sr = scaler.scale_near_anchor(&mut u2, &anchor, &data).unwrap();
                println!("  轮{}: 能量不足→扩展+{:.0}E +{}节点", i, sr.energy_expanded_by, sr.nodes_added);
                if let Ok(mem) = MemoryCodec::encode(&mut u2, &anchor, &data) {
                    mems2.push(mem); datasets2.push(data);
                }
            }
        }
    }
    ...
    for (i, mem) in mems2.iter().enumerate() {
        if let Ok(decoded) = MemoryCodec::decode(&u2, mem) {
            let err = datasets2[i].iter().zip(decoded.iter()).map(|(a, b)| (a - b).abs()).fold(0.0f64, f64::max);
            max_err2 = max_err2.max(err);
        }
    }
    println!("  结果: {}记忆 节点{} 总能{:.0} 精度{:.2e} 守恒:{} ({:.1}ms)",
        mems2.len(), u2.active_node_count(), u2.total_energy(), max_err2,
        if u2.verify_conservation() { "✓" } else { "✗" }, ms);

    // ═══ 3. frontier_expansion前沿扩展 ═══
    println!("\n━━━ 3. frontier_expansion前沿扩展 ═━━");
    let t = Instant::now();
    let mut u3 = DarkUniverse::new(10_000_000.0);
    u3.materialize_biased(Coord7D::new_even([0, 0, 0, 0, 0, 0, 0]), 50.0, 0.6).unwrap();
    ...
    for round in 1..=8 {
        ...
        println!("  扩展{}: +{}节点 → 总{} 连通分量:{}", round, sr.nodes_added, u3.active_node_count(), topo.connected_components);
    }
    let ms = t.elapsed().as_secs_f64() * 1000.0;
    println!("  守恒: {} ({:.0}ms)", if u3.verify_conservation() { "✓" } else { "✗" }, ms);

    // ═══ 4. scale_up触发阈值 ═══
    println!("\n━━━ 4. scale_up触发阈值 ═══");
    let configs = [
        ("默认(80%)", AutoScaleConfig::default()),
        ("敏感(60%)", AutoScaleConfig { scale_up_threshold: 0.60, ..AutoScaleConfig::default() }),
        ("迟钝(95%)", AutoScaleConfig { scale_up_threshold: 0.95, ..AutoScaleConfig::default() }),
    ];
    for (name, config) in configs {
        ...
        for i in 0..20i32 {
            u4.materialize_biased(Coord7D::new_even([i, 0, 0, 0, 0, 0, 0]), 40.0, 0.6).ok();
        ...
        println!("  {}: {}节点 利用率{:.1}% 触发{}次 守恒:{}",
            name, u4.active_node_count(), u4.stats().utilization * 100.0, triggers,
            if u4.verify_conservation() { "✓" } else { "✗" });
    }
    ...
    for i in 0..100i32 {
        ...
        u5.materialize_biased(c, 50.0, if i % 3 == 0 { 0.8 } else { 0.02 }).ok();
    }
    let before = u5.stats();
    println!("  回收前: {}节点 具现{} 暗{}", before.active_nodes, before.manifested_nodes, before.dark_nodes);

    let sr = scaler.scale_down(&mut u5, &[]);
    let after = u5.stats();
    let ms = t.elapsed().as_secs_f64() * 1000.0;
    println!("  回收后: {}节点 具现{} 暗{} 删除{}", after.active_nodes, after.manifested_nodes, after.dark_nodes, sr.nodes_removed);
    println!("  守恒: {} ({:.1}ms)", if u5.verify_conservation() { "✓" } else { "✗" }, ms);
    ...
    for cycle in 0..20 {
        for i in 0..5i32 {
            ...
            if let Ok(mem) = MemoryCodec::encode(&mut u6, &anchor, &data) {
                h6.record_path(&[*mem.anchor(), Coord7D::new_even([cycle * 100 + i * 20 + 10, 0, 0, 0, 0, 0, 0])], 1.0);
            }
        }
        ...
        if !actions.is_empty() && actions[0].action_type != tetramem_v12::universe::observer::RegulatorActionType::None {
            if actions.iter().any(|a| a.action_type == tetramem_v12::universe::observer::RegulatorActionType::ExpandEnergy) {
    ...
    println!("  20轮自调节后:");
    println!("    节点:{} 利用率:{:.1}% 健康:{}", health.node_count, health.energy_utilization * 100.0, health.health_level().as_str());
    println!("    赫布边:{} 平均权重:{:.2}", health.hebbian_edge_count, health.hebbian_avg_weight);
    println!("    守恒: {} ({:.0}ms)", if u6.verify_conservation() { "✓" } else { "✗" }, ms);
    ...
    for i in 0..20i32 {
        ...
        if let Ok(mem) = MemoryCodec::encode(&mut u7, &anchor, &data) {
            ...
            if i > 0 { h7.record_path(&[*mems7[i as usize - 1].anchor(), *mem.anchor()], 1.5); }
        }
    }
    ...
    let ms = t.elapsed().as_secs_f64() * 1000.0;
    println!("  应激:{:.2} 熵:{:.3} 不平衡:{:.2} 动作:{}",
        report.stress_level, report.entropy, report.dimension_pressure.imbalance, report.actions.len());
    ...
    for d in 0..7 {
        ...
        println!("    dim{}: {:.0} {}", d, report.dimension_pressure.dims[d], bar);
    }
    println!("  守恒: {} ({:.0}ms)", if u7.verify_conservation() { "✓" } else { "✗" }, ms);

    // ═══ 8. 极限压力: 持续记忆注入直到系统饱和 ═══
    println!("\n━━━ 8. 极限压力: 持续注入直到饱和 ═━━");
    let t = Instant::now();
    let mut u8 = DarkUniverse::new(100_000.0);
    u8.materialize_biased(Coord7D::new_even([0,0,0,0,0,0,0]), 50.0, 0.6).ok();
    ...
    for round in 0..5000i32 {
        let data: Vec<f64> = (0..7).map(|j| ((round * 7 + j) % 100) as f64 * 0.1).collect();
    ...
    for (i, mem) in mems8.iter().enumerate() {
        if let Ok(decoded) = MemoryCodec::decode(&u8, mem) {
            let err = datasets8[i].iter().zip(decoded.iter()).map(|(a, b)| (a - b).abs()).fold(0.0f64, f64::max);
    ...
    println!("  注入5000轮:");
    println!("    成功记忆:{}  失败编码:{}  扩展事件:{}", mems8.len(), failed_encodes, scale_events);
    println!("    能量: 100K→{:.0}K (×{:.1})  节点:{}", u8.total_energy() / 1000.0, u8.total_energy() / 100_000.0, stats8.active_nodes);
    println!("    利用率:{:.1}%  赫布边:{}", stats8.utilization * 100.0, h8.edge_count());
    println!("    记忆可读:{}/{} 精度:{:.2e} 损坏:{}", ok_mems, mems8.len(), max_decode_err, corrupt);
    println!("    守恒: {} ({:.0}ms)", if u8.verify_conservation() { "✓" } else { "✗" }, ms);

    // ═══ 总结 ═══
    println!("\n{}", "═".repeat(60));
    println!("  自我扩展性评估:");
    println!("  ┌─────────────────────────────────────────────────┐");
    println!("  │ 1K轮: {}记忆 精度{:.1e} 能量×{:.0}          │", mems.len(), max_err, u.total_energy() / 10000.0);
    println!("  │ 5K轮: {}记忆 精度{:.1e} 能量×{:.0}       │", mems8.len(), max_decode_err, u8.total_energy() / 100_000.0);
